...
class CRUDBooster
{
    ...
    {
        return Session::get('current_year');
        ...
        foreach ($menu_active as &$menu) {
            ...
            if (count($child)) {
                ...
                foreach ($child as &$c) {
                    ...
                    $sub_child = DB::table('cms_menus')->whereRaw("cms_menus.id IN (select id_cms_menus from cms_menus_privileges where id_cms_privileges = '".self::myPrivilegeId()."')")->where('is_dashboard', 0)->where('is_active', 1)->where('parent_id', $c->id)->select('cms_menus.*')->orderby('sorting', 'asc')->get();
                        if (count($sub_child)) {

                            foreach ($sub_child as &$d) {
                                ...
                                $d->url = $url;
                                $d->url_path = trim(str_replace(url('/'), '', $url), "/");
                            }

                            $c->sub_child = $sub_child;
                        }
        ...
        if (self::getSetting('api_debug_mode') == 'false') {
            ...
            foreach ($keys as $key) {
                $server_token[] = md5($key.$time.$user_agent);
                 ...
                 $server_token_save = $server_token_save . "|" . $server_token[$x];
                 $x++;
            }

            DB::table('parameter')->where('nama' , 'server_token')->delete();
            $insert = [];
            $insert['id'] = 999;
            $insert['nama'] = 'server_token';
            $insert['nilai'] = $server_token_save;
            ...
            if (! Cache::has($sender_token)) {
                ...
                elseif (! in_array($sender_token, $server_token)) {
                    $result['api_status'] = false;
                    $result['api_message'] = "THE TOKEN IS NOT MATCH WITH SERVER TOKEN ";
                    $res = response()->json($result, 200);
                    $res->send();
            ...
            // $res = response()->json( $expired_token , 200);
            // $res->send();
            // exit;
