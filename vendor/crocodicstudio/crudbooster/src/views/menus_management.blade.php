                                            ...
                                            <li data-id='{{$child->id}}' data-name='{{$child->name}}'>
                                                ...
                                                <ul>    
                                                    @if($child->sub_children)
                                                        @foreach($child->sub_children as $sub_child)
                                                            @php
                                                                $privileges = DB::table('cms_menus_privileges')
                                                                ->join('cms_privileges','cms_privileges.id','=','cms_menus_privileges.id_cms_privileges')
                                                                ->where('id_cms_menus',$sub_child->id)->pluck('cms_privileges.name')->toArray();
                                                            @endphp
                                                            <li data-id='{{$sub_child->id}}' data-name='{{$sub_child->name}}'>
                                                                <div class='{{$sub_child->is_dashboard?"is-dashboard":""}}'
                                                                    title="{{$sub_child->is_dashboard?'This is setted as Dashboard':''}}"><i
                                                                            class='{{($sub_child->is_dashboard)?"icon-is-dashboard fa fa-dashboard":$sub_child->icon}}'></i> {{$sub_child->name}}
                                                                    <span class='pull-right'><a class='fa fa-pencil' title='Edit'
                                                                                                href='{{route("MenusControllerGetEdit",["id"=>$sub_child->id])}}?return_url={{urlencode(Request::fullUrl())}}'></a>&nbsp;&nbsp;<a
                                                                                title="Delete" class='fa fa-trash'
                                                                                onclick='{{CRUDBooster::deleteConfirm(route("MenusControllerGetDelete",["id"=>$sub_child->id]))}}'
                                                                                href='javascript:void(0)'></a></span>
                                                                    <br/><em class="text-muted">
                                                                        <small><i class="fa fa-users"></i> &nbsp; {{implode(', ',$privileges)}}</small>
                                                                    </em>
